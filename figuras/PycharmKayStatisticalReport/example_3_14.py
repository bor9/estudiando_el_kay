...
f0 = np.linspace(xmin, xmax, 5000)
alpha = 4 * math.pi * f0
beta = 0
f1 = np.zeros(f0.shape)
for n in np.arange(N):
    f1 += n * np.cos(alpha * n + beta)
f1 /= (N ** 2)

f2_a = np. sin(alpha * (2 * N - 1) / 2 + beta) / (2 * N * np.sin(alpha / 2))
f2_b = - np.sin(alpha * N / 2 + beta) * np.sin(alpha * N / 2) / (2 * (N ** 2) * np.square(np.sin(alpha / 2)))

f2 = f2_a + f2_b

diff_f = f1 - f2
print(np.amax(np.abs(diff_f)))
...
plt.plot(f0, f2, color='k', linewidth=2)

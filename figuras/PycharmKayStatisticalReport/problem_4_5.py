...
m = np.linspace(xmin, xmax, 1000)
signal1 = np.cos(math.pi * m * (N - 1) / N) * np.sin(math.pi * m) / (np.sin(math.pi * m / N))
Ms = np.arange(-M, M + 1)
signal2 = np.cos(math.pi * Ms * (N - 1) / N) * np.sin(math.pi * Ms) / (np.sin(math.pi * Ms / N))
...
ymax_ax = np.amax(signal1) * 1.2
ymin_ax = np.amin(signal1) * 1.2
...
plt.plot(m, signal1, color='k', linewidth=2)
plt.plot(Ms, signal2, 'r.', markersize=4)
